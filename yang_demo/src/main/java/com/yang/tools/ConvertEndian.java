...
/**
 * @ClassName: ConvertEndian
 * @package com.yang.tools
 * @Description: 网络字节顺序转换 BIG-ENDIAN就是低位字节排放在内存的高端，高位字节排放在内存的低端；
 *               LITTLE-ENDIAN就是低位字节排放在内存的低地址端，高位字节排放在内存的高地址端。
 *               网络字节序，JAVA字节序是BIG-ENDIAN。
 * 
 * @author Mr.yang
 * @date 2016年1月24日 下午2:57:43
 * @version V1.0
 */
public class ConvertEndian {
	/**
	 * 通信格式转换
	 * 
	...
	public static int reverseInt(int i) {
		int result = ConvertEndian.hBytesToInt(ConvertEndian.toLH(i));
		return result;
	}
	...
	public static short reverseShort(short s) {
		short result = ConvertEndian.hBytesToShort(ConvertEndian.toLH(s));
		return result;
	}
	...
	public static float reverseFloat(float f) {
		float result = ConvertEndian.hBytesToFloat(ConvertEndian.toLH(f));
		return result;
	}
}
